...
{
	//TODO: Ejercicio 2
	unsigned int group_number = Ext2FS::blockgroup_for_inode(inode_number);
	unsigned int index_number = Ext2FS::blockgroup_inode_index(inode_number);
	Ext2FSBlockGroupDescriptor* group_descriptor = Ext2FS::block_group(group_number);
	unsigned int inode_table_pointer = group_descriptor->inode_table;
	unsigned int inodes_per_block = (1024 <<_superblock->log_block_size) / _superblock->inode_size;
	unsigned int inode_block = index_number / inodes_per_block;
	unsigned int inode_block_index = index_number % inodes_per_block;
	Ext2FSInode* inodeArray = malloc(1024 <<_superblock->log_block_size);
	Ext2FS::read_block(inode_table_pointer + inode_block, inodeArray);
	return inodeArray[inode_block_index];
	//Falta averiguar en que bloque esta el inodo, levantarlo a un buffer y recorrerlo linealmente por inode_number % cant_inodos_por_bloque	
...
{
	if(block_number < 12) {
		inode->blocks[block_number];
	}
	else  {
		char* buff = malloc(sizeof(struct Ext2FSSuperblock));
		Ext2FS::read_block(2, buff);
		Ext2FSSuperblock* superB = (struct Ext2FSSuperblock*) buff;
		unsigned int amount_of_pointers_in_block_size = 1024 << (superB->log_block_size - 2); //cantidad de punteros a bloque que hay en la tabla
		int* blockBuffer = malloc(amount_of_pointers_in_block_size << 2);
		if ( block_number < amount_of_pointers_in_block_size + 12 ) {
			//indireccion simple
			Ext2FS::read_block(blocks[12], blockBuffer);
			return blockBuffer[block_number - 12];
		}
		else if ( block_number < amount_of_pointers_in_block_size * amount_of_pointers_in_block_size  + amount_of_pointers_in_block_size +  12_) 
		{
			//indireccion doble
			Ext2FS::read_block(blocks[13], blockBuffer);
			unsigned int block_number_in_table = block_number - amount_of_pointers_in_block_size - 12;
			unsigned int number_of_indirect_table =  block_number_in_table / amount_of_pointers_in_block_size;
			unsigned int offset_in_indirect_table = block_number_in_table % amount_of_pointers_in_block_size;
			Ext2FS::read_block(blockBuffer[number_of_indirect_table], blockBuffer);

			return blockBuffer[offset_in_indirect_table];
		}
	}
